...
import { getOnboardingModules, getWatchedVideoIds, getSeancesProgress, getObjectif, getJoursEntrainement } from "@/lib/tts";
import { ttsObjectifLabel } from "@/lib/tts-objectifs";
...
import TtsSignOutButton from "./TtsSignOutButton";
import TtsManageSubscriptionButton from "./TtsManageSubscriptionButton";
import TtsUpgradeTeaser from "@/components/TtsUpgradeTeaser";
import TtsStreakFlame from "@/components/TtsStreakFlame";
import TtsJoursEntrainement from "./TtsJoursEntrainement";
import { ttsColors } from "@/lib/tts-theme";

export const dynamic = "force-dynamic";

export default async function TtsProfilPage() {
  ...
  return (
    <div style={{ backgroundColor: "#0D0D0D", minHeight: "100vh", paddingBottom: 100 }}>
      ...
      <div className="mx-auto" style={{ maxWidth: 480 }}>
        <TtsHeader variant="page" title="Profil" subtitle={email || undefined} />

        <div style={{ padding: "20px 20px 100px" }}>
          <div style={{ background: ttsColors.card, border: `1px solid ${ttsColors.cardBorder}`, borderRadius: 16, padding: 18, display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              ...
              <span
                className="font-body"
                style={{ display: "inline-block", marginTop: 6, background: "rgba(0,0,0,0.35)", border: "1px solid rgba(230,57,70,0.4)", color: ttsColors.redBright, fontSize: 10, letterSpacing: "1px", padding: "3px 9px", borderRadius: 20, fontWeight: 700 }}
              >
                TIME TO START
              </span>
            </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
            <div style={{ flex: 1, background: ttsColors.card, border: `1px solid ${ttsColors.cardBorder}`, borderRadius: 16, padding: "14px 16px", textAlign: "center" }}>
              <TtsStreakFlame days={streakInfo.streak_current} />
              <p className="font-body" style={{ margin: "4px 0 0", color: "#fff", fontSize: 18, fontWeight: 700 }}>{streakInfo.streak_current}</p>
              <p className="font-body" style={{ margin: 0, color: ttsColors.muted, fontSize: 11 }}>jour{streakInfo.streak_current > 1 ? "s" : ""} de suite</p>
            ...
            <div style={{ flex: 1, background: ttsColors.card, border: `1px solid ${ttsColors.cardBorder}`, borderRadius: 16, padding: "14px 16px", textAlign: "center" }}>
              ...
              <p className="font-body" style={{ margin: 0, color: ttsColors.muted, fontSize: 11 }}>séance{seancesValidees > 1 ? "s" : ""} validée{seancesValidees > 1 ? "s" : ""}</p>
            </div>
          </div>

          {objectif && (
            <div style={{ background: ttsColors.card, border: `1px solid ${ttsColors.cardBorder}`, borderRadius: 16, padding: "14px 18px", marginTop: 12 }}>
              <p className="font-body" style={{ margin: 0, color: ttsColors.muted, fontSize: 12 }}>Ton objectif</p>
              <p className="font-body" style={{ margin: "4px 0 0", color: "#fff", fontSize: 14, fontWeight: 600 }}>{ttsObjectifLabel(objectif) ?? objectif}</p>
            </div>
          )}

          <TtsJoursEntrainement initialJours={joursEntrainement} />

          {dateDebutLabel && (
            <div style={{ background: ttsColors.card, border: `1px solid ${ttsColors.cardBorder}`, borderRadius: 16, padding: "14px 18px", marginTop: 12 }}>
              <p className="font-body" style={{ margin: 0, color: ttsColors.muted, fontSize: 12 }}>Date de démarrage</p>
