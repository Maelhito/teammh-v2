...
function getModuleContent(slug: string): string | null {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("PDF:: ")) {
      ...
      if (!filePath || filePath === "#") {
        ...
        html.push(`<a href="${filePath}" download class="pdf-block pdf-block--active"><span class="pdf-icon">↓</span><span class="pdf-title">${processInline(title)}</span><span class="pdf-badge">Télécharger</span></a>`);
      }
      i++; continue;
    }
    ...
    if (line.startsWith(">")) {
      ...
      const inner = bqLines.map((l) => `<p>${processInline(l)}</p>`).join("");
...
export default async function ModulePage({ params }: PageProps) {
  ...
  const rawContent = getModuleContent(slug);
