...
import { getTtsStripeOffer } from "@/lib/tts-stripe-offers";
...
export async function POST(request: NextRequest) {
  ...
  const offerConfig = getTtsStripeOffer(offer);
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${origin}/tts?checkout=success`,
      cancel_url: `${origin}/tts/paiement?checkout=canceled`,
