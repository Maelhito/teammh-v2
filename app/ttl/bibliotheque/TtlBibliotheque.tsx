...
import type { TtlProgramme, TtlProgrammeVideo, TtlRecette, TtlCapsule, TtlRecetteCategorie, TtlSeanceProgress } from "@/lib/ttl";
import { TtlFilterChip, TtlRowCard, TtlNewBadge } from "@/components/TtlUI";
...
export default function TtlBibliotheque({ initialTab, current, previous, future, recettes, capsules, seancesProgress, initialSemaine }: Props) {
  ...
  const [openCapsule, setOpenCapsule] = useState<TtlCapsule | null>(null);
  ...
  return (
    <div style={{ padding: "20px 20px 100px" }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
        <TtlFilterChip active={tab === "seances"} onClick={() => setTab("seances")}>Séances</TtlFilterChip>
        <TtlFilterChip active={tab === "recettes"} onClick={() => setTab("recettes")}>Recettes</TtlFilterChip>
        <TtlFilterChip active={tab === "capsules"} onClick={() => setTab("capsules")}>Capsules</TtlFilterChip>
      </div>
      ...
      {tab === "recettes" && (
        <div>
          ...
          {filteredRecettes.length === 0 && (
            <p className="font-body" style={{ color: ttlColors.muted, fontSize: 13 }}>Aucune recette dans cette catégorie.</p>
          )}
        </div>
      )}
        ...
        <div>
          {capsules.map((c, i) => (
            <TtlRowCard
              ...
              badge={i === 0 ? <TtlNewBadge /> : undefined}
            />
          ))}
          {capsules.length === 0 && (
            <p className="font-body" style={{ color: ttlColors.muted, fontSize: 13 }}>Aucune capsule pour l&apos;instant.</p>
          )}
        </div>
      )}
