-- ============================================================
-- Nettoyage des fuseaux hérités — à exécuter APRÈS sql/fuseau_horaire.sql
-- ============================================================
--
-- Constat au 21 août 2026 : sur 36 abonnements push, 27 dataient d'avant le
-- correctif du 15/07/2026 qui captait le vrai fuseau de l'appareil. Tous
-- portaient 'Europe/Paris' — la valeur par défaut codée en dur dans
-- /api/push/subscribe, pas une détection. Signe qui ne trompe pas : aucune
-- détection à Bali, alors que l'équipe y vit.
--
-- Le backfill de la migration précédente a promu ces valeurs en source de
-- vérité. On les retire ici. Les personnes concernées retombent sur le repli
-- (Pacific/Noumea) jusqu'à ce qu'elles rouvrent l'app : SyncFuseau écrit alors
-- leur vrai fuseau, en France comme ailleurs.
--
-- Les identifiants sont écrits en dur volontairement : ils viennent d'une
-- analyse ligne à ligne, et aucun compte hors de cette liste ne peut être
-- touché par erreur.

BEGIN;

-- ── 1. Les copies héritées sur push_subscriptions ────────────────────────────
-- Sans ça, la chaîne de repli de lib/temps.ts ressusciterait 'Europe/Paris'.
...
-- ── 2. Les 21 profils sans aucune détection réelle ────────────────────────────
-- Remis à NULL : repli sur Pacific/Noumea, puis vraie détection à la prochaine
-- ouverture de l'app.
UPDATE user_profiles
SET    timezone = NULL, timezone_updated_at = NULL
WHERE  user_id IN (
...
-- ── 3. Le profil dont la vraie détection avait été écrasée ───────────────────
-- Ce compte a un ancien enregistrement 'Europe/Paris' ET une détection récente
-- à Nouméa. Le backfill avait retenu la mauvaise ; on remet la bonne.
UPDATE user_profiles SET timezone = 'Pacific/Noumea', timezone_updated_at = NOW()
WHERE  user_id = 'dc6256fe-9f46-4c26-bd2f-55a27d347787';

COMMIT;

-- ── Vérification (à lancer après) ────────────────────────────────────────────
-- Attendu : 2 profils avec un fuseau (1 Europe/Paris réel, 1 Pacific/Noumea),
-- les autres à NULL en attente de détection.
-- SELECT timezone, count(*) FROM user_profiles GROUP BY timezone ORDER BY 2 DESC;
