...
CREATE TABLE IF NOT EXISTS offres_clientes (
  ...
  offre TEXT NOT NULL CHECK (offre IN ('TTL', 'TTM', 'TTL')),
...
CREATE TABLE IF NOT EXISTS offres_clientes_historique (
  ...
  offre_avant TEXT CHECK (offre_avant IN ('TTL', 'TTM', 'TTL')),
  offre_apres TEXT NOT NULL CHECK (offre_apres IN ('TTL', 'TTM', 'TTL')),
  hors_ordre BOOLEAN DEFAULT false,
  -- true si la transition ne respecte pas l'ordre normal TTL -> TTM -> TTL
  confirmed_by TEXT,
